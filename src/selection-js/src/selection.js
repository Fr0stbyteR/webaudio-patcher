...
function Selection(options = {}) {
    ...
    const that = {
        ...
        _target: doc.body,
        _init() {
            let targetNodes = doc.querySelectorAll(that.options.targetDOM);
            if (targetNodes.length) that._target = targetNodes[0];
            // Append area to body
            that._target.appendChild(that._areaElement);
        ...
        _onTapStart(evt) {
            let {x, y, target} = _.simplifyEvent(evt);
            const bRect = that._target.getBoundingClientRect();
            x -= bRect.left;
            y -= bRect.top;
        ...
        _updateArea(evt) {
            ...
            if (x < 0) x = 0;
            if (y < 0) y = 0;
            if (x > brect.width) x = brect.width;
            if (y > brect.height) y = brect.height;
